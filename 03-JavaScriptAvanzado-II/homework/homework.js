...
function counter() {
  ...
  debe guardar el argumento junto con el resultado de la invocación (tip: usá un objeto donde cada propiedad sea el argumento, y su 
    valor el resultado de la correspondiente invocación a cb) de manera que, la próxima vez que reciba el mismo argumento, no sea
  ...
  */
  function cacheFunction(cb) {//                  cb seria n*n
   var cache={}//                                  la memoria donde se alojan resultados
   return function(arg){
    if(cache.hasOwnProperty(arg)){
      return cache[arg];//                si el resultado esta en el objeto lo devuelve
    }else {//                                       si no esta
      cache[arg]= cb(arg)
      return cache[arg];
    }
